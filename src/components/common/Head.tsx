  ...
  const handleSearch = (search: string): void => {
    ...
    if (regex.test(search)) {
      ...
      if (outageId!==undefined) {
        window.location.href = configData.HOME_URL + '/outage/' + outageId; //FIXME: use router instead
      } else {
      ...
      <nav className="navbar navbar-expand-lg fixed-top navbar-dark bg-dark">
        <div className="container-fluid">
          ...
          <div className="navbar-collapse offcanvas-collapse" id="navbarsExampleDefault">
            ...
            {(isLogged && props.form) && props.form}
          </div>
        </div>
      </nav>
      <div className="nav-scroller bg-body shadow-sm">
        <nav className="nav">
          <a className="nav-link active" title="Listes toutes les perturbations actuelles du système d'information" href={configData.HOME_URL}>Perturbations</a>
          ...
          {isLogged && <>
            <a className="nav-link" title="Vous assiste afin d'exprimer votre besoin de support de la meilleure manière" href={configData.HOME_URL + "/how-to"}><InfoCircle /> Assistant</a>
            <a className="nav-link ms-auto text-success fw-bold" href={configData.HOME_URL + "/contribute"}><LightbulbFill /> J'améliore !</a>
            <a className="nav-link" href={configData.HOME_URL + "/dashboard"}><Speedometer2 /> Notre performance</a>
            <a className="nav-link" href={configData.HOME_URL + "/profile"}><Person /> Mon profil</a>
          </>}
        </nav>
      </div>
    </>
